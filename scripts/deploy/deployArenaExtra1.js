...
const CONTRACT_ADDR_MAP = {
  ...
  ArenaExtra1: {
    ...
    address: "0x1BD40F89665fa378BA598568d37488274a415eF8",
  },
  ArenaExtra2: {
...
const RELY_ON_MINTER_MAP = [{ contract: "Coin", params: ["ArenaExtra1"] }];
...
async function deployArenaExtra1() {
  const contract = await deployContract("HorseArenaExtra", "ArenaExtra1");
  const init = await contract.init(
    ContractMap.get("ArenaAttr").address,
    ContractMap.get("Coin").address,
    ContractMap.get("HorseCourseAttrOpera").address,
    ContractMap.get("RaceAttr1").address,
    ContractMap.get("RaceAttr1_1").address,
    ContractMap.get("RaceAttr2").address,
    ContractMap.get("RaceAttr2_1").address,
    ContractMap.get("HorseToken").address,
    ContractMap.get("ArenaToken").address,
    ContractMap.get("Constant").address,
    ContractMap.get("MetaToken").address,
    ContractMap.get("RaceToken").address
  );
  await init.wait();
  await contract.setDistribute(ContractMap.get("RaceBonusDistribute").address);
  return contract;
}
